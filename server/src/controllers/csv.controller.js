...
export const uploadCsv = catchAsync(async (req, res) => {
  ...
  if (fileName !== 'Consultar-Horário.csv') {
    throw new ApiError(httpStatus.BAD_REQUEST, 'O nome do arquivo deve ser exatamente "Consultar-Horário.csv".');
  }
  ...
  res.status(httpStatus.CREATED).json({ message: 'Upload e processamento realizados com sucesso.' });
});
